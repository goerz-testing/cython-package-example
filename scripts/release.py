...
def make_release(package_name):
    ...
    check_git_clean()
    new_version = ask_for_release_version(package_name, test=True)
...
def get_pypi_versions(package_name, test=False):
    ...
    If `test` is True, query the test.pypi.org instead of the main PyPI.
    """
...
def edit(filename):
    """Open filename in ``$EDITOR`` (vi by default)."""
    editor = os.getenv('EDITOR', 'vi')
    if click.confirm("Open %s in %s?" % (filename, editor), default=True):
        run([editor, filename], check=True)
...
def list_versions(package_name, test=False):
    ...
    If `test` if True, query test.pypi.org instead of the main PyPI.
    """
    try:
        pypi_versions = get_pypi_versions(package_name, test=test)
    except OSError:
...
def propose_next_version(dev_version):
    """Return the most likely release version based on the current version."""
...
def ask_for_release_version(package_name, test=False):
    """Ask for the version number of the release.

    The version number is checked to be a valid next release. The default is
    based on which versions already exist as tags locally and/or on PyPI (or
    test.pypi.org if `test` is True).
    """
    dev_version = get_version(join('.', 'src', package_name, '__init__.py'))
    proposed_version = propose_next_version(dev_version)
    released_versions = list_versions(package_name, test=test)
...
def check_dist():
    """Quietly make dist and check it.

    This is mainly to ensure that the README and HISTORY metadata are
    well-formed.
    """
